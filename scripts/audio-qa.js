...
for(const it of items){
  if(!it.id)errors.push('manifest item missing id');
  else if(ids.has(it.id))errors.push('duplicate audio id: '+it.id);
  else ids.add(it.id);
  ...
  if(m&&it.relatedLetter&&m[1].toUpperCase()!==String(it.relatedLetter).toUpperCase())warnings.push(it.id+': filename and relatedLetter mismatch');
}
const cfg=loadCfg(cfg2,'PHONICS_LEVEL2_CLIPS');
const refs=[],usedFiles=new Set();
for(const [rowName,row] of Object.entries(cfg)){
  const file=strip(row.audio||''); usedFiles.add(file);
  const labels=Object.keys(row.clips||{});
  refs.push({rowName,file,labels});
  ...
  if(!item)warnings.push('referenced audio file missing from manifest: '+file);
  else{
    const missing=labels.filter(x=>!(item.clipLabels||[]).includes(x));
    const extra=(item.clipLabels||[]).filter(x=>!labels.includes(x));
...
const canProbe=has('ffprobe'),technical=[];
if(canProbe){for(const f of new Set([...usedFiles,...manifestFiles])){const p=path.join(game,f);if(fs.existsSync(p)){const d=durMs(p);technical.push({file:f,durationMs:d});if(!d||d<=0)errors.push('silent or zero duration file: '+f)}}}
const report={summary:{manifestItems:items.length,gameReferencedAudioFiles:usedFiles.size,gameReferencedRows:refs.length,errors:errors.length,warnings:warnings.length,reviewRequired:review.length,ffprobe:canProbe},errors,warnings,reviewRequired:review,technical};
